...
export default function NotasView({ user }) {
        ...
        <div className="flex-1 flex flex-col overflow-hidden min-w-0">
          <div className="shrink-0 flex items-center gap-3 px-5 py-3 border-b border-white/10 bg-black/20">
            <input value={activeNote.titulo || ''} onChange={e => updateNote({ titulo: e.target.value })}
              className="flex-1 bg-transparent text-white font-black text-lg outline-none placeholder-white/25 min-w-0" placeholder="Sem título"/>
          ...
          {activeNote.tipo === 'canvas' ? (
            <CanvasEditor key={activeNote.id} noteId={activeNote.id} data={activeNote.canvas_data || { nodes: [], edges: [] }} allNotes={notes} onChange={canvas_data => updateNote({ canvas_data })} onOpenNote={setActiveId}/>
          ) : (
            <TextEditor key={activeNote.id} noteId={activeNote.id} conteudo={activeNote.conteudo || ''} allNotes={notes} onChange={conteudo => updateNote({ conteudo })} onOpenNote={setActiveId}/>
          )}
        </div>
      ) : !loading && (
